...
func TestGetPreferencia(t *testing.T) {
	...
	createdAt := time.Now()
	requestImplementer.NewMPPayer(id, name, email, areaCode, cellNumber, identificationNumber, createdAt)
	...
	currencyID := ""
	unitPrice := 0.0
	requestImplementer.NewMPItems(id, title, description, pictureUrl, categoryID, int(quantity), currencyID, unitPrice)
